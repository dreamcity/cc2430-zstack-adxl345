...
//  osal_event_hdr_t *pMsg;
  ...
  if ( events & AXD_PULSE_MSG_EVT )
  {
    ...
    // return unprocessed events
    return (events ^ AXD_PULSE_MSG_EVT);
  }
...
void ReadAdcValue( uint8 flag )
{
    ...
    if (flag == ADC_TEMP)
  {
    ...
    BUFFER[2] = (INT8U)(value + '0');
      ...
      if (timer_flag >= 5){
          value = (float)pulse_cnt / (float)timer_flag * 60;
      }
      if (timer_flag >= 60){
          pulse_cnt = 0;
          timer_flag = 1;
      }
      BUFFER[7] = (INT8U)(value / 100 + '0');
      value = value % 100;
      BUFFER[6] = (INT8U)(value / 10 + '0');
      value = value % 10;
      BUFFER[5] = (INT8U)(value + '0');
  }
}
